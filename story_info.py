...
class StoryBot:
    def __init__(self, player):
        self.player = player
        self.progress = 0
        self.already_pushed_story_progress = 0
    ...
    def progress_next_story(self):  # story method를 실행한 뒤에 해줄것 - 아직 분기점은 없기에 한 progress만 있으면 충분
        self.progress+=1

    def progress_check(self,progress_value):  # 각 story를 실행하기 전에 조건에 부합하는지 체그해야함. 그때 반드시 확인해 볼 조건.
        if self.progress == progress_value and self.already_pushed_story_progress != self.progress:  # 이미 실행된 스토리가 아니어야 한다! (이미 한 말 또하면 안되니까)
            return True
        return False
    ...
    def update_story(self):  # 이게 조건 확인하는 복잡한 함수
        ...
        if self.progress_check(1):
            self.process_story_txt_and_push(self.story_lines[0])
            self.already_pushed_story_progress = self.progress
...
/[HarryPotter 또는 Gandalph의 방에 진입한 적이 있을때]
...
/[Bad end: progress 수치가 충분히 오르지 않은 상태에서 탈출]
후후... 탈출이다! 
이제 나를 막는건 아무것도 없어!!!
'''
